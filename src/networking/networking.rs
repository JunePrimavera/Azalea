...
pub fn ssh_command(address : Address, username: &str, command: &str) -> Result<Vec<u8>, Vec<u8>> {
    ...
    let tcp : TcpStream = TcpStream::connect(format!("{}:{}", host, port)).map_err(|e| format!("Failed to connect: {}", e))?;
    let mut sess : Session = Session::new().unwrap();
    sess.set_tcp_stream(tcp);
    sess.handshake().map_err(|e| format!("SSH handshake failed: {}", e))?;
    sess.userauth_agent(username).map_err(|e| format!("Failed to authenticate: {}", e))?;
